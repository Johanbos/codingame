...
class Cockpit
{
    ...
    {
        var delta = DistanceToLandingZone();
        return Land(delta.Y);
        ...
        if (goingToLandingZone)
        {
            ...
            {
                return FlyHorizontallyTowardsLanding(delta.X);
            }
        }
        else
        {
    ...
    private string Turn()
    {
        ...
        var directionToLanding = Lander.Position.X < LandingZone.Middle.X ? 1 : -1;
        var angle = 60 * directionToLanding;
        var thrust = Lander.Rotate * Lander.Direction() >= 0 ? Lander.MaxThrustPower : 0;
        return WithinLimits(angle, thrust);
    }

    private string Land(int distanceY)
    {
        ...
        else
        {
            ...
            var distanceToSafeLandingSpeed = DistanceToDecelerate(Math.Abs(Lander.VerticalSpeed), Lander.MaxThrustPower - Map.Gravity, Lander.LandingVerticalSpeed);
            Console.Error.WriteLine("Distance to safe landing speed: " + distanceToSafeLandingSpeed);
            if (distanceY <= distanceToSafeLandingSpeed)
            {
    ...
    private string FlyHorizontallyTowardsLanding(int distanceX)
    {
        Console.Error.WriteLine($"Mode: FlyHorizontallyTowardsLanding");
        var predictedLanding = PredictedLanding();
        var idealSpeed = predictedLanding.NearX(LandingZone.Middle) ? Lander.MaxLandingHorizontalSpeed : Lander.MaxFlyingHorizontalSpeed;

        // Keep speed within flying limits. Increase angle if speed is low, decrease angle if speed is near ideal speed. 
        var speedDiff = idealSpeed - Math.Abs(Lander.HorizontalSpeed);
        var angle = speedDiff;
        Console.Error.WriteLine("Horizontal speed diff: " + speedDiff + "m/s");

        // Since the angle is not 0 not all thrust compensates for gravity, calculate the added trust needed to to compensate for gravity.
        var thrust = Map.Gravity / (float)Math.Cos(angle * Math.PI / 180) - Lander.VerticalSpeed;

        // Go left vs right
        angle *= Lander.Direction() * -1;
        return WithinLimits(angle, thrust);
    }

    private Point PredictedLanding()
    {
        // Assuming the correct angle needs to be set, how much meters is needed to reduce the speed to finalSpeed.
        var direction = Lander.Direction();
        var x = Lander.Position.X + Lander.HorizontalSpeed + (DistanceToDecelerate(Math.Abs(Lander.HorizontalSpeed), 2, Lander.MaxLandingHorizontalSpeed) * direction);
        var y = Lander.Position.Y + Lander.VerticalSpeed + DistanceToDecelerate(Math.Abs(Lander.VerticalSpeed), Lander.MaxThrustPower - Map.Gravity);
        Console.Error.WriteLine("Predicted landing position at: " + x + "," + y);
        Console.Error.WriteLine("Actual landingzone middle at: " + LandingZone.Middle.X + "," + LandingZone.Middle.Y);
        return new Point(x, y);
    }

    internal int DistanceToDecelerate(int initialSpeed, float deceleration, int finalSpeed = 0)
    {
        ...
        if (finalSpeed > initialSpeed) throw new ArgumentOutOfRangeException(nameof(finalSpeed), "Final speed must be less than or equal to initial speed.");

        // Assuming the correct angle is already set, how much meters is needed to reduce the speed to finalSpeed.
    ...
    internal string WithinLimits(float angle, float thrust)
    {
        Console.Error.WriteLine($"Commanding angle: {angle}, thrust: {thrust}");
        var angleBounded = (int)Math.Round(Math.Max(-60, Math.Min(60, angle)), 0);
    ...
    {
        var distanceX = Math.Abs(Lander.Position.X - LandingZone.Middle.X);
        var distanceY = Math.Abs(Lander.Position.Y - LandingZone.Middle.Y);
...
internal class Lander
{
    ...
    public const int MaxFlyingHorizontalSpeed = 40;
    ...
    internal void Update(int x, int y, int horizontalSpeed, int verticalSpeed, int fuel, int rotate, int power)
    {
    ...
    internal bool GoingToLandingZone(Line landingZone)
    {
        ...
        {
            -1 => Position.X > landingZone.Middle.X,
            0 => true,
            _ => Position.X < landingZone.Middle.X
        };
    }
}
